...
{
    [TestFixture]
    public class MonthlyPaymentFrequencyPolicyTest
    {
        [Test]
        public void GetNextDate_ReturnsNextMonth()
        {
            var date = new DateTime(2013, 6, 7);
            var policy = new MonthlyPaymentFrequencyPolicy();
        ...
        public void GetNumberOfDays_Returns28to31()
        {
            ...
            var policy = new MonthlyPaymentFrequencyPolicy();
            Assert.AreEqual(31, policy.GetNumberOfDays(january));
            Assert.AreEqual(29, policy.GetNumberOfDays(februaryLeap));
            Assert.AreEqual(28, policy.GetNumberOfDays(february));
