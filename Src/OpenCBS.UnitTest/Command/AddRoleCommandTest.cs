...
namespace OpenCBS.UnitTest.Command
{
    ...
    public class AddRoleCommandTest
    {
        ...
        private AddRoleCommand _command;
        ...
        public void SetUp()
        {
            ...
            _command = new AddRoleCommand(_presenter, _roleService, _appController);
        }

        [Test]
        public void Execute_CommandResultIsNotOk_DoesNotAddRole()
        {
            _presenter.Get(Arg.Any<RoleDto>()).Returns(new Result<RoleDto>(CommandResult.Cancel, null));
            _command.Execute(new AddRoleData());
            _roleService.DidNotReceive().Add(Arg.Any<RoleDto>());
            _appController.DidNotReceive().Raise(Arg.Any<RoleSavedEvent>());
        }

        [Test]
        public void Execute_CommandResultIsOk_AddsRole()
        {
            _presenter.Get(Arg.Any<RoleDto>()).Returns(new Result<RoleDto>(CommandResult.Ok, new RoleDto()));
            _command.Execute(new AddRoleData());
            _roleService.Received().Add(Arg.Any<RoleDto>());
            _appController.Received().Raise(Arg.Any<RoleSavedEvent>());
        }
    }
}
