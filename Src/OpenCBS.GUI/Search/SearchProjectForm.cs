        ...
        private void _CheckTiersAndDisplayIt(ProjetSearchResult result)
        {
            try
            {
                ...
                if (result.Type == OClientTypes.Person)
                        ((ITempMainView)_mdiForm).InitializePersonForm((Person)_client,_projet);
                if (result.Type == OClientTypes.Group)
                    ((ITempMainView) _mdiForm).InitializeGroupForm((Group) _client, _projet);
                if (result.Type == OClientTypes.Corporate)
                    ((ITempMainView) _mdiForm).InitializeCorporateForm((Corporate) _client, _projet);
