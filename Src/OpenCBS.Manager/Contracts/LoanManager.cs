    ...
    public class LoanManager : Manager
    {
        ...
        public decimal CalculatePenaltiesForDate(int contractId, DateTime date)
        {
            const string q = @"SELECT al.id
                            , CASE
	                            WHEN penalties_accrued IS NULL
	                            THEN 0
	                            ELSE penalties_accrued
	                            END AS penalties_accrued
                            , CASE
	                            WHEN penalties_paid IS NULL
	                            THEN 0
	                            ELSE penalties_paid
	                            END AS penalties_paid
                            FROM dbo.ActiveLoans(@date, 0) AS al
                            LEFT JOIN 
                            (	
	                            SELECT ce.contract_id, SUM(lp.penalty) AS penalties_accrued
	                            FROM dbo.LoanPenaltyAccrualEvents lp
	                            LEFT JOIN dbo.ContractEvents ce ON ce.id=lp.id
	                            WHERE ce.is_deleted=0
	                            GROUP BY ce.contract_id
                            ) AS accrued_penalty ON accrued_penalty.contract_id = al.id
                            LEFT JOIN
                            (
	                            SELECT ce.contract_id, 
	                            SUM(re.penalties) AS penalties_paid
	                            FROM dbo.RepaymentEvents re
	                            LEFT JOIN dbo.ContractEvents ce ON ce.id=re.id
	                            WHERE ce.is_deleted=0
	                            GROUP BY ce.contract_id
                            ) AS paid_penalty ON paid_penalty.contract_id = al.id
                            WHERE id = @contractId";
            using (var connection = GetConnection())
            using (var cmd = new OpenCbsCommand(q, connection))
            {
                cmd.AddParam("contractId", contractId);
                ...
                using (OpenCbsReader r = cmd.ExecuteReader())
                {
                    if (r.Read())
                    {
                        var penalty = r.GetDecimal("penalties_accrued") - r.GetDecimal("penalties_paid");
